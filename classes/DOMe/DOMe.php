...
if (!class_exists("DOMe")) {
	class DOMe implements Object_Array {
		public $tag, $children, $attributes, $text, $allowHTML, $endTag, $id, $preValue, $defaultTidy;
		...
		function setClass($class) {
                ...
                function removeClass($class) {
                    ...
                    foreach ($classes as $key => $cls) {
                        if ($cls == $class) unset($classes[$key]);
                    }
                ...
                function importHTML($html, $root = true) {
                    ...
                    while (strlen($html) > 0) {
                        ...
                        if (substr_count($html, "<") > 0) {
                            ...
                            if (strlen($text) > 0) $this->newChild("", $text, false, false);
                            ...
                            if (substr($html, 0, 1) == "/") {
                                ...
                                if ($endTag) {
                                        ...
                                        $html = $element->importHTML($html, false);
                                }
                            }
                        }
                        else {
		...
		function generate($tidy = true, $indent = 0, $indentStr = "  ") {
                    ...
                    if ($this->defaultTidy === false || $this->defaultTidy === true) {
                        $tidy = $this->defaultTidy;
                    }
                    ...
                    foreach ($this->childIndex as $index) {
                        if (is_object($this->children[$index]) && (get_class($this->children[$index]) == "DOMe" || is_subclass_of($this->children[$index], "DOMe"))) {
                            $data .= $this->children[$index]->generate($tidy, $indent + 1, $indentStr);
                        }
                    }

                    // Draw the end tag?
                    if ($this->endTag && !empty($this->tag)) {
                        $data .= "{$indentText}</{$this->tag}>";
                        if ($tidy) {
                            $data .= $tidyText;
                        }
